...
export function ChatView() {
  ...
  async function handleSend(text, attachments = []) {
    ...
    // Images need to go to the vision endpoint as real file data — the old
    // code just stringified the filename, which is why the AI always said
    // it couldn't see images (it never actually received any).
    const imageAttachment = attachments.find((a) => a.type === "image");
    const codeAttachments = attachments.filter((a) => a.type !== "image");

    let fullMessage = text;
    if (codeAttachments.length > 0) {
      const fileParts = codeAttachments.map(
    ...
    try {
      /*
       * The old Base44 version called:
       *
       *   db.functions.invoke("chatWithAi", ...)
       *
       * Vercel now handles the request through /api/chat (text) or
       * /api/ChatUpload (image, as multipart form data).
       *
       * IMPORTANT:
       * The API key is NOT sent from the browser. It belongs in the
       * Vercel server-side environment variables.
       *
       * user_id is sent so Iris can keep separate persistent memory
      ...
      if (imageAttachment) {
        ...
        formData.append("file", imageAttachment.file, imageAttachment.name);
        formData.append("message", fullMessage);
        formData.append("user_id", user?.id || "anonymous");

        res = await fetch("/api/ChatUpload", {
          method: "POST",
          body: formData,
        });
      } else {
      ...
      if (!res.ok) {
        console.error("Iris API error:", res.status, data);
        throw new Error(
          data?.error ||
          data?.detail ||
      ...
      console.error("Failed to reach Iris API:", e);
      answer =
    ...
    if (isAuthed) {
      ...
      const newCredits = Math.max(
        0,
        Math.round((credits - creditsUsed) * 100) / 100
      );
      ...
      try {
        ...
        console.error("Could not update credits:", e);
      }
    }
